...
carro_velho = pygame.image.load('maraca.png')
...
#    m = x+(car_x/2)-3
 #   n = y - 10
    for p in range( count_balas+1 ):
        if n[p] > -30:
            n[p] -= 2
        else:
            n[p] = -30
        print(m[p], n[p])
        pygame.draw.rect(surface, (200, 100, 50), pygame.Rect(m[p], n[p], 5, 20), 4) #testes aqui
